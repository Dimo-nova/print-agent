...
| Reloj | Deriva calculada con la cabecera `Date` de las respuestas de PostgREST. El umbral de 2 minutos y `claimed_at` usan `Date.now() + deriva`. Eso cubre lo que se compara con el servidor, **no** el refresco del token: supabase-js lo programa con el reloj local, así que `systemd-timesyncd` (activo por defecto en Raspberry Pi OS) sigue siendo requisito. Una sesión que muere sin avisar la recoge el perro guardián: 10 min sin un `fetchClaimable` correcto → `exit 1` y systemd reinicia. |
| Reintentos | Backoff `5 s, 15 s, 45 s, 2 min, 5 min ×6` por impresora. Al décimo fallo el job pasa a `failed` con `error`: ~35 min de margen para reiniciar una impresora o volver a enchufar un cable. La espera vive en el worker de la impresora (`pausedUntil`) y se respeta **antes** de coger el siguiente trabajo, así que frena también al que reofrece el poll de 60 s; dormir después de procesar dejaba que el reoferto entrase en una cola vacía y quemase el intento siguiente al instante. El KDS es la red de seguridad. |
